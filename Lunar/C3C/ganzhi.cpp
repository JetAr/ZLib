...
int  mGz(int y,int m,int d,int h,int calType)
{
    ...
    int  sD0=revD0(y,floor(sDt0),calType);
    int  sM0=floor(sD0/100);
    int  sDate0=sD0%100+tail(sDt0);

    int  sN1=2*m-1;
    int  sDt1=S(y,sN1,1,calType);
    int  sD1=revD0(y,floor(sDt1),calType);
    int  sM1=floor(sD1/100);
    int   sDate1=sD1%100+tail(sDt1);

    int  sN2=2*m;
    int  sDt2=S(y,sN2,1,calType);
    int  sD2=revD0(y,floor(sDt2),calType);
    int  sM2=floor(sD2/100);
    ...
    int  sD3=revD0(y,floor(sDt3));
    int  sM3=floor(sD3/100);
...
{
    int  v=12*gan(dGz(y,m,d,h,calType))+floor((h+1)/2)-11;
    if(h==23)
        v-=12;
